...
public class Copy {
	...
	public static void main(String[] args) {
		...
		if (args.length != 2)//매개 변수 확인
		{
			...
			System.err.println("Usage : Copy <Source> <destinations>");
		}
		else
		{
	...
	public static void copy(String from_name, String to_name) throws IOException
	{
		...
		//to_name이 디렉토리이면, 소스 파일 이름을 destination 파일 이름으로 그대로 사용
		if(to_file.isDirectory())
			to_file = new File(to_file, from_file.getName());
		
		//destination 파일이 존재하면, 엎어 쓰기 전에 쓰기가 가능한지를 확인
		//destination 파일 존재하지 않으면, 디렉토리가 존재하고 쓰기가 가능한지 확인
		if(to_file.exists())
		{
			if(!to_file.canWrite())
				abort("쓰기가 불가능합니다. : " +to_name);
			//덮어 쓸 것인지를 지정(이미 존재한다면)
			...
			if(parent == null)
				parent = System.getProperty("user.dir");//null인 경우, 현재 디렉토리를..
			...
			if(!dir.canWrite())
				abort("파일에 쓰기 방지가 되어 있네요" +parent);
		...
		//예외가 발생하였더라도 스트림을 항상 달아 주어야 함
		finally {
			if (from != null) //객체가 있다면 (empty check!)
			try {
				from.close();
			} catch (IOException e) {;}
				if(to != null)
